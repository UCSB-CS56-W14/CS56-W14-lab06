package edu.ucsb.cs56.w14.drawings.ziping_he.advanced;
import java.awt.geom.GeneralPath; // combinations of lines and curves
import java.awt.geom.AffineTransform; // translation, rotation, scale
import java.awt.Shape; // general class for shapes

// all imports below this line needed if you are implementing Shape
import java.awt.geom.Point2D; 
import java.awt.geom.Line2D; 
import java.awt.geom.Rectangle2D;
import java.awt.Rectangle;
import java.awt.geom.PathIterator;
import java.awt.geom.AffineTransform;

import edu.ucsb.cs56.w14.drawings.utilities.ShapeTransforms;
import edu.ucsb.cs56.w14.drawings.utilities.GeneralPathWrapper;

/**
   A vector drawing of a house that implements
   the Shape interface, and so can be drawn, as well as
   rotated, scaled, etc.
      
   @author Phill Conrad 
   @author Ziping He
   @version for CS56, Winter 14, UCSB
   
*/
public class Flag extends GeneralPathWrapper implements Shape
{
    /**
       Constructor

       @param x x coord of upper left corner of flag
       @param y y coord of upper left corner of flag
       @param width width of the flag
       @param height height of the flag
     */
    public Flag(double x, double y, double width , double height)
    {
    
        // Rather than having to scale at the end, we can just
        // draw things the right way to begin with, using the
        // x, y, width, height.   If you haven't already
        // hard coded a particular drawing, this may be an easier
        // way.
        

        double flagHeight = height;
        
        double flagLowerLeftY = y - flagHeight;
        
        // Make the flag
        
        Rectangle2D.Double flagShape = 
	  new Rectangle2D.Double(x, flagLowerLeftY ,
				 width, flagHeight);
       
        GeneralPath wholeFlag = this.get();
        wholeFlag.append(flagShape, false);
          
    }

}
